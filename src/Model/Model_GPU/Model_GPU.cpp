  ...
  accelerationsf3(n_particles)
{
    ...
    cudaMalloc(&positionsGPU,     n_particles * sizeof(float4));
    cudaMalloc(&velocitiesGPU,    n_particles * sizeof(float3));
    cudaMalloc(&accelerationsGPU, n_particles * sizeof(float3));

    cudaMemcpy(positionsGPU, positionsf4.data(),
               n_particles * sizeof(float4), cudaMemcpyHostToDevice);
